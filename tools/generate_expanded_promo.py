...
SLIDES_DIR = os.path.join(OUTPUT_DIR, "slides_v2")
...
# 16 slides total (x2 duration ~ 76s, covering full mod features and alternate history)
slides_data = [
    {
        "id": "slide_01",
    ...
    {
        "id": "slide_15",
        "bg": "mod_page/assets/mockups/main_menu_mockup.jpg",
        "title": "КУЛЬТОВЫЙ САУНДТРЕК «МОНГОЛ ШУУДАН»",
        "subtitle": "8 легендарных анархо-рок боевиков, газетная хроника и озвучка вступительного манифеста",
        "badge": "НЕПОВТОРИМАЯ АТМОСФЕРА СТЕПИ",
        "duration": 5.0
    },
    {
        "id": "slide_16",
...
def make_slide(slide):
    ...
    odraw.rectangle([card_left, card_top, card_right, card_bot], fill=(15, 13, 11, 210), outline=(195, 155, 90, 240), width=3)
    
    im = Image.alpha_composite(im.convert("RGBA"), overlay).convert("RGB")
    draw = ImageDraw.Draw(im)
    
    font_badge = ImageFont.truetype(FONT_BOLD, 25)
    font_title = ImageFont.truetype(FONT_BLACK, 50)
...
print("Generating 16 slide images...")
for s in slides_data:
    make_slide(s)

# Compile clips with fast ffmpeg preset
clip_files = []
...
raw_video = os.path.join(OUTPUT_DIR, "raw_video_v2.mp4")
cmd_concat = [
    "ffmpeg", "-y",
    "-f", "concat",
    "-safe", "0",
...
cmd_final = [
    ...
    "-map", "0:v",
    "-map", "1:a",
    "-af", f"afade=t=in:st=0:d=1.0,afade=t=out:st={total_dur-2.5}:d=2.5",
    "-c:v", "copy",
    "-c:a", "aac",
    "-b:a", "256k",
    "-shortest",
...
# Copy to brain for immediate artifact preview
...
print(f"SUCCESS! Extended 76s Trailer created at: {FINAL_VIDEO} and {BRAIN_PROMO}")
