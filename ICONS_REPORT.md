...
**Метод проверки.** Иконки фокусов сверены с ванильным `interface/goals.gfx` (дампы **1.7.1 Hydra** и **1.14.10**) и не требуют DLC. Все национальные духи используют только 15 тематических категорий собственного GLP-пака из `interface/GLP_ideas.gfx`; точное соответствие контролируется файлом `tools/idea_pictures.tsv` и автоматическим аудитом. Иконки Kaiserreich и DLC-контент не используются. Иконки шаблонов дивизий сверяются с названиями дивизий отдельной проверкой `check_division_icons` (`tools/glp_audit.py`) по реестру `tools/division_icons.tsv`. Правило итерации 23: силуэт **рода войск** берётся только из графики базовой игры (свой DDS рода войск — ошибка аудита); собственные плитки мод рисует там, где ванильного ресурса нет, — это знамёна добровольцев 90/91.
...
| Шаблоны дивизий | 15 | **2 собственные плитки (знамёна 90/91)** | 4 шаблона на ванильном силуэте (counter 92), 9 — по составу |

## Иконки шаблонов дивизий: конница обязана показывать коня

**Механика HOI4** (`hoi4.paradoxwikis.com/Division_modding`, `/Effect#division_template`):
иконка шаблона — это `GFX_div_templ_N_large` (плитка **76×42**, дизайнер дивизий
и детальная панель) и `GFX_div_templ_N_small` (**30×12**, фишка на карте). Номер `N`
задаётся в шаблоне полем `template_counter`; **если поля нет, движок берёт силуэт
под-юнита с наибольшим `priority`** среди линейных батальонов (колонки
`combat_support` — артполки, ПТО, ПВО — род войск не определяют).

**Корень бага.** Ванильные приоритеты (`common/units/*.txt`, дамп 1.14.1):

| Батальон | `priority` | Что рисует |
|---|---|---|
| `light_armor` | 2501 | танк |
| `medium_armor` / `heavy_armor` / `modern_armor` | 2502…2510 | танк |
| `bus` | 1000 | грузовик |
| `mechanized` | 610 | грузовик |
| `marine`, `mountaineers` | 601 | морпех, горец |
| `infantry` | 600 | пехотинец |
| `cavalry`, `motorized` | 599 | **конь**, грузовик |
| `militia`, `irregular_infantry` | 400 | ополченец |

Тачаночный курень — 4 × `cavalry` + 2 × `motorized` + **1 × `light_armor`**
(бронеавтомобиль в ударной конной части — по ТЗ). Танковый батальон выигрывает
сравнение 2501 > 599, и конная дивизия показывала **танк**. То же касалось
«Трофейного отряда РПА» (2 × `infantry` + 2 × `cavalry` → пехота при конной
группе имён).

**Исправление.** Составы не тронуты (геймплей тот же) — род войск закрепляется
явным `template_counter`, ровно как мод уже делал для добровольцев (90/91):

| `counter` | Плитки | Что рисует | Шаблоны |
|---|---|---|---|
| 90 | `GLP_white_volunteers.dds` (76×42 + 30×12, ARGB8888) | знамя белогвардейских добровольцев | «Офицерская добровольческая дивизия» |
| 91 | `GLP_spanish_volunteers.dds` (76×42 + 30×12, ARGB8888) | знамя CNT-FAI | «Интернациональная бригада CNT-FAI» |
| **92** | **`gfx/interface/counters/divisions_large/unit_cavalry_icon.dds`** + **`divisions_small/onmap_unit_cavalry_icon.dds`** — графика базовой игры, `noOfFrames = 2` | **конь, а не танк — силуэт кавалерии из ванильной игры** | «Ударный тачаночный курень им. Батьки Махно», «Трофейный отряд РПА», «Казачья добровольческая конная дивизия», «Испанская ударная конная колонна» |

Силуэт 92 **не собирается** модом: `GFX_div_templ_92_{large,small}` указывают на
те же файлы базовой игры, что использует ванильный `interface/subuniticons.gfx`
(`GFX_unit_cavalry_icon_medium` / `_medium_white`), включая `noOfFrames = 2` —
атласы силуэтов ванили лежат двумя кадрами (обычный и приглушённый), и без этого поля
движок сжал бы оба кадра в одну плитку. Наша прежняя плитка `GLP_tachanka.dds`
(76×42 и 30×12, мастера `tools/_icons_src/_src_div_cavalry*.png`, плоская фигура
цвета `#517151` с верхней световой растяжкой и тёмной обводкой) удалена вместе
со строками сборки: самодельный силуэт рода войск расходится с ванильным при
каждом патче игры, а нужен именно ванильный знак кавалерии.
`tools/build_division_icons.sh` оставлен для 90/91 — там своего ресурса в ванили
нет (обводка и растяжка на 30×12 там отключаются, иначе силуэт превращается в
пятно: покрытие мелкой плитки 16 %, крупной 37,6 %).

Знамёна 90/91 род войск намеренно не утверждают, поэтому для них соответствие
«название ↔ иконка» не проверяется — но стоят они теперь только у **пехотных**
добровольческих шаблонов: конное название обязано давать коня и на карте, и в
дизайнере дивизий.

**Полное соответствие 15 шаблонов (проверка `check_division_icons`).**

| Шаблон | Состав | Иконка, которую возьмёт движок | Название / группа имён | OK |
|---|---|---|---|---|
| Ударный тачаночный курень им. Батьки Махно | 4 × конница, 2 × мотопехота, 1 × лёгкие танки | конь — ванильный `unit_cavalry_icon.dds` (counter 92) | конная, `GLP_CAV` | ✔ (был танк) |
| Трофейный отряд РПА | 2 × пехота, 2 × конница | конь — ванильный силуэт (counter 92) | конная, `GLP_CAV` | ✔ (была пехота) |
| Казачья добровольческая конная дивизия | 4 × конница, 2 × мотопехота, 1 × лёгкие танки | конь — ванильный силуэт (counter 92) | конная, `GLP_CAV` | ✔ (было знамя 90) |
| Испанская ударная конная колонна | 4 × конница, 2 × мотопехота, 1 × лёгкие танки | конь — ванильный силуэт (counter 92) | конная, `GLP_CAV` | ✔ (было знамя 91) |
| Офицерская добровольческая дивизия | 7 × пехота, 2 × артполк | знамя 90 | пехотная, `GLP_INF` | ✔ знамя |
| Интернациональная бригада CNT-FAI | 7 × пехота, 2 × артполк | знамя 91 | пехотная, `GLP_INF` | ✔ знамя |
| Штурмовая дивизия Чёрного Знамени | 7 × пехота, 2 × артполк | пехотинец (`infantry` 600) | пехотная, `GLP_INF` | ✔ |
| Стрелковая бригада вольных коммун | 9 × пехота | пехотинец | пехотная, `GLP_INF` | ✔ |
| Степной моторизованный полк «Воля або Смерть» | 6 × мотопехота, 1 × артполк | грузовик (`motorized` 599) | моторизованная, `GLP_MOT` | ✔ |
| Бронетанковая дивизия Черной Гвардии | 4 × лёгкие танки, 4 × мотопехота | танк (`light_armor` 2501) | бронетанковая, `GLP_ARM` | ✔ |
| Трофейный ремонтно-ударный отряд РПА | 2 × пехота, 2 × мотопехота, 1 × лёгкие танки | танк | бронетанковая, `GLP_ARM` | ✔ |
| Черноморская вольная морская бригада | 6 × морпеха, 1 × артполк | морпех (`marine` 601) | морская, `GLP_MAR` | ✔ |
| Горно-повстанческая бригада «Орлиное Гнездо» | 6 × горные стрелки | горец (`mountaineers` 601) | горная, `GLP_MNT` | ✔ |
| Партизанский заслон народной самообороны | 4 × пехота, 2 × конница | пехотинец | самооборона, `GLP_GAR` | ✔ |
| Ударная Добровольческая Бригада | 4 × пехота, 2 × конница | пехотинец | пехотная, `GLP_INF` | ✔ |

Плюс контроль самих списков имён: каждое имя в `GLP_*_HISTORICAL` обязано
упоминать род своей группы (иначе дивизия «конной» группы получала бы имя
танкиста). Найденное расхождение исправлено: «7-я Крымская бригада матросов и
повстанцев» в стрелковой группе → «7-я Крымская **стрелковая** бригада матросов
и повстанцев».
